...
class KVContract extends Contract {
  ...
  async uploadRecord(ctx, recordId, recordTitle, tags, metadata, auditors) {
    const newRecord = {
      docType: 'record',
      recordId,
      recordTitle,
    ...
    const newRecordBuffer = Buffer.from(JSON.stringify(newRecord));
    await ctx.stub.putState(recordId, newRecordBuffer);

    return { success: "OK" };
  }

  async grantAccess(ctx, recordId, customerId, paymentTxId) {
    ...
    const [recordBuffer, grantData] = await Promise.all([ctx.stub.getState(recordId), this.GetQueryResultForQueryString(ctx, JSON.stringify(queryString))]);

    let alreadyHasAccess = grantData.some(grant => grant.customerId === customerId && grant.recordId === recordId);
    if (!recordBuffer || recordBuffer.length === 0) {
      throw new Error(`The record with ID ${recordId} does not exist`);
    }
    if (alreadyHasAccess) {
      throw new Error(`The customer with ID ${customerId} already has an access to the record with ID ${recordId}`);
    }
  ...
  async getRecordList(ctx) {
    ...
    const recordList = await this.GetQueryResultForQueryString(ctx, JSON.stringify(queryString));
    const recordListToReturn = recordList.map(record => ({
      id: record.key,
  ...
  async getRecord(ctx, recordId, customerId) {
    const queryString = {
      selector: {
        docType: 'grant',
        recordId,
    ...
    const hasAccess = grantData.some(grant => grant.customerId === customerId && grant.recordId === recordId);
    if (hasAccess) {
